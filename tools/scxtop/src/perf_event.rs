...
pub fn read_file_u64<P: AsRef<Path>>(path: P) -> Result<u64> {
    ...
    // Trim whitespace (especially newline characters) before parsing
...
impl PerfEvent {
    ...
    pub fn attach(&mut self) -> Result<()> {
        ...
        attrs.set_inherit(1);
        attrs.set_pinned(1);
        // attrs.read_format = PERF_FORMAT_TOTAL_TIME_ENABLED | PERF_FORMAT_TOTAL_TIME_RUNNING;
        // attrs.sample_type = PERF_SAMPLE_ID;
...
pub fn available_perf_events() -> Result<BTreeMap<String, HashSet<String>>> {
    ...
    for line in reader.lines() {
        ...
        // Extract the event name (assuming it's the first word)
