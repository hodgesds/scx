...
static u64 layer_dsq_id(u32 layer_id, u32 llc_id)
{
...
int consume_open_non_preempt(struct cost *costc, u32 my_llc_id)
{
	struct layer *layer;
	u64 dsq_id;
	u32 idx, llc_idx, layer_idx;
...
void BPF_STRUCT_OPS(layered_dispatch, s32 cpu, struct task_struct *prev)
{
	...
	/* consume preempting layers first */
	if (consume_preempting(costc, my_llc_id))
		return;
	...
	/* consume !open layers second */
	if (consume_non_open(costc, cpu, my_llc_id))
		return;

	/* consume !preempting open layers */
	if (consume_open_non_preempt(costc, my_llc_id))
		return;
