...
pick_llc:
	...
	if (chosen->idle_cpumask)
		cpu = bpf_cpumask_any_and_distribute(cast_mask(chosen->idle_cpumask),
						     cast_mask(chosen->smt_cpumask));
	if (cpu >= 0) {
		*is_idle = true;
		return cpu;
	}
	...
	if (chosen->cpumask &&
	    (cpu = bpf_cpumask_any_distribute(cast_mask(chosen->idle_cpumask))) >= 0) {
		*is_idle = true;
		return cpu;
	}
			 ...
			 s32 prev_cpu, u64 wake_flags, bool *is_idle)
{
	...
	// if (taskc->all_cpus) {
	// 	// Interactive tasks go for the last idle cpu.
	...
	// 		}
	// 	}
	// 	// if (llcx->smt_cpumask && llcx->idle_cpumask && llcx->cpumask &&
	// 	//     (cpu = bpf_cpumask_any_and_distribute(cast_mask(llcx->idle_cpumask),
	// 	// 				  cast_mask((smt_enabled && !interactive) ?  llcx->smt_cpumask: llcx->cpumask))) &&
	// 	//     cpu < nr_cpus && llcx->idle_cpumask) {
	// 	// 	cpu = prev_cpu;
	// 	// 	*is_idle = bpf_cpumask_test_and_clear_cpu(cpu, llcx->idle_cpumask);
	// 	// 	goto found_cpu;
	// 	// }
	// }
	...
	if (!taskc->all_cpus) {
		...
		if (smt_enabled && llcx->idle_cpumask && llcx->smt_cpumask) {
			cpu = bpf_cpumask_any_and_distribute(cast_mask(llcx->idle_cpumask),
							     cast_mask(llcx->smt_cpumask));
			if (cpu < nr_cpus && llcx->idle_cpumask) {
				*is_idle = bpf_cpumask_test_and_set_cpu(cpu, llcx->idle_cpumask);
				goto found_cpu;
			}
		}
		...
		if (llcx->idle_cpumask &&
		    (cpu = bpf_cpumask_any_distribute(cast_mask(llcx->idle_cpumask)))) {
			if (cpu < nr_cpus && llcx->idle_cpumask) {
				*is_idle = bpf_cpumask_test_and_set_cpu(cpu, llcx->idle_cpumask);
				goto found_cpu;
			}
	...
	if (wake_flags & SCX_WAKE_SYNC) {
		...
		if (waker_taskc->llc_id == llcx->id || !wakeup_llc_migrations) {
			...
			if (llcx->idle_cpumask && llcx->smt_cpumask &&
			    (cpu = bpf_cpumask_any_and_distribute(cast_mask(llcx->idle_cpumask), cast_mask(llcx->smt_cpumask)))) {
				if (cpu < nr_cpus && llcx->idle_cpumask &&
		...
		if (waker_llcx->smt_cpumask && waker_llcx->idle_cpumask &&
		    (cpu = bpf_cpumask_any_and_distribute(cast_mask(waker_llcx->idle_cpumask), cast_mask(waker_llcx->smt_cpumask))) &&
		    cpu < nr_cpus) {
		...
		if (waker_llcx->cpumask && waker_llcx->idle_cpumask &&
		    (cpu = bpf_cpumask_any_and_distribute(cast_mask(waker_llcx->idle_cpumask), cast_mask(waker_llcx->cpumask))) &&
		    cpu < nr_cpus) {
			stat_inc(P2DQ_STAT_WAKE_MIG);
			if (llcx->idle_cpumask)
				*is_idle = bpf_cpumask_test_and_clear_cpu(cpu, llcx->idle_cpumask);
	...
	// 		if (llcx->idle_cpumask &&
	// 		    (cpu = bpf_cpumask_any_and_distribute(cast_mask(llcx->idle_cpumask), cast_mask(llcx->big_cpumask))) >= 0) {
	// 			*is_idle = true;
	// 			goto found_cpu;
	// 		}
	// 	}
	...
	if (!interactive &&
	    llcx->cpumask && llcx->idle_cpumask &&
	    (cpu = bpf_cpumask_any_and_distribute(cast_mask(llcx->idle_cpumask), cast_mask(llcx->cpumask))) &&
	    cpu < nr_cpus) {
		if (llcx->idle_cpumask)
			*is_idle = bpf_cpumask_test_and_clear_cpu(cpu, llcx->idle_cpumask);
		goto found_cpu;
