...
static __always_inline s32 __pick_idle_cpu(struct bpf_cpumask *mask,
                                           int flags) {
...
static int llc_create_atqs(struct llc_ctx *llcx) {
  ...
  if (topo_config.nr_llcs > 1) {
    llcx->mig_atq =
        (scx_atq_t *)scx_atq_create_size(false, topo_config.nr_cpus);
    if (!llcx->mig_atq) {
      scx_bpf_error("ATQ failed to create ATQ for LLC %u", llcx->id);
      return -ENOMEM;
    }
    trace("ATQ mig_atq %llu created for LLC %llu", (u64)llcx->mig_atq,
          llcx->id);
  }
...
static inline void stat_inc(enum stat_idx idx) { stat_add(idx, 1); }
...
static bool can_migrate(task_ctx *taskc, struct llc_ctx *llcx) {
  if (topo_config.nr_llcs < 2 || !taskc->all_cpus ||
...
static __always_inline void update_vtime(struct task_struct *p,
                                         struct cpu_ctx *cpuc, task_ctx *taskc,
                                         struct llc_ctx *llcx) {
  /*
...
static s32 pick_idle_affinitized_cpu(struct task_struct *p, task_ctx *taskc,
  ...
  if (llcx->node_cpumask && mask) {
    bpf_cpumask_and(mask, cast_mask(llcx->node_cpumask), p->cpus_ptr);
...
static s32 pick_idle_cpu(struct task_struct *p, task_ctx *taskc, s32 prev_cpu,
  ...
  if (wake_flags & SCX_WAKE_SYNC) {
    ...
    if (taskc->interactive ||
      ...
      if (llcx->cpumask && (cpu = __pick_idle_cpu(llcx->cpumask, 0)) >= 0) {
    ...
    if (waker_taskc->llc_id == llcx->id || !lb_config.wakeup_llc_migrations) {
    ...
    if (waker_llcx->cpumask &&
        (cpu = __pick_idle_cpu(waker_llcx->cpumask, SCX_PICK_IDLE_CORE)) >= 0) {
 ...
 * - P2DQ_ENQUEUE_PROMISE_FIFO: The completer should enqueue this task on a fifo
 * dsq.
 * - P2DQ_ENQUEUE_PROMISE_VTIME: The completer should enqueue this task on a
 * vtime dsq.
...
static __always_inline void async_p2dq_enqueue(struct enqueue_promise *ret,
   ...
   * into the local DSQ.
   */
  if (p2dq_config.kthreads_local && (p->flags & PF_KTHREAD) &&
      p->nr_cpus_allowed == 1) {
    stat_inc(P2DQ_STAT_DIRECT);
    u64 slice_ns = clamp_slice(scale_by_task_weight(p, dsq_time_slices[0]));
  ...
  if (!taskc->all_cpus || (p->cpus_ptr == &p->cpus_mask &&
                           p->nr_cpus_allowed != topo_config.nr_cpus)) {
    if (!bpf_cpumask_test_cpu(cpu, p->cpus_ptr))
      cpu = pick_idle_affinitized_cpu(p, taskc, cpu, &ret->has_cleared_idle);
    else
    ...
    trace("ENQUEUE %s weight %d slice %llu vtime %llu llc vtime %llu", p->comm,
          p->scx.weight, taskc->slice_ns, p->scx.dsq_vtime, llcx->vtime);
  ...
  if (p2dq_config.select_idle_in_enqueue &&
    ...
    cpu = pick_idle_cpu(p, taskc, cpu, 0, &ret->has_cleared_idle);
    if (!(cpuc = lookup_cpu_ctx(cpu)) ||
        !(llcx = lookup_llc_ctx(cpuc->llc_id))) {
      scx_bpf_error("invalid lookup");
      return;
...
static __always_inline void complete_p2dq_enqueue(struct enqueue_promise *pro,
  ...
  case P2DQ_ENQUEUE_PROMISE_ATQ_FIFO:
    ...
    if (!ret) {
      ...
      scx_bpf_dsq_insert(p, pro->vtime.dsq_id, pro->vtime.slice_ns,
                         pro->vtime.enq_flags);
  ...
  case P2DQ_ENQUEUE_PROMISE_ATQ_VTIME:
    ...
    ret = scx_atq_insert_vtime(pro->vtime.atq, (u64)p->pid, pro->vtime.vtime);
    if (!ret) {
      // The ATQ was full, fallback to the DSQ.
      scx_bpf_dsq_insert_vtime(p, pro->vtime.dsq_id, pro->vtime.slice_ns,
                               pro->vtime.vtime, pro->vtime.enq_flags);
...
static __always_inline int p2dq_running_impl(struct task_struct *p) {
  ...
  if (!(taskc = lookup_task_ctx(p)) || !(cpuc = lookup_cpu_ctx(task_cpu)) ||
      !(llcx = lookup_llc_ctx(cpuc->llc_id)))
    return -EINVAL;
  ...
  if (taskc->llc_id != cpuc->llc_id) {
    ...
    trace("RUNNING %d cpu %d->%d llc %d->%d", p->pid, cpuc->id, task_cpu,
...
void BPF_STRUCT_OPS(p2dq_stopping, struct task_struct *p, bool runnable) {
  ...
  if (!(taskc = lookup_task_ctx(p)) || !(llcx = lookup_llc_ctx(taskc->llc_id)))
    return;
  ...
  if (taskc->was_nice && (cpuc = lookup_cpu_ctx(scx_bpf_task_cpu(p)))) {
  ...
  if (!runnable) {
    ...
    if (used >= ((9 * last_dsq_slice_ns) / 10)) {
      if (taskc->dsq_index < p2dq_config.nr_dsqs_per_llc - 1 &&
        ...
        trace("%s[%p]: DSQ inc %llu -> %u", p->comm, p, taskc->last_dsq_index,
              taskc->dsq_index);
      } else {
      ...
      if (taskc->dsq_index > 0) {
        taskc->dsq_index -= 1;
        stat_inc(P2DQ_STAT_DSQ_CHANGE);
        trace("%s[%p]: DSQ dec %llu -> %u", p->comm, p, taskc->last_dsq_index,
              taskc->dsq_index);
      } else {
        stat_inc(P2DQ_STAT_DSQ_SAME);
      }
...
static __always_inline bool consume_llc(struct llc_ctx *llcx) {
  ...
  if (p2dq_config.atq_enabled && scx_atq_nr_queued(llcx->mig_atq) > 0) {
    ...
    trace("ATQ %llu insert %s->%d", llcx->mig_atq, p->comm, p->pid);
    scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL, taskc->slice_ns, taskc->enq_flags);
    bpf_task_release(p);
...
static __always_inline int dispatch_pick_two(s32 cpu, struct llc_ctx *cur_llcx,
  ...
  if (topo_config.nr_llcs == 1 || lb_config.dispatch_pick2_disable ||
  ...
  right =
      topo_config.nr_llcs == 2 ? lookup_llc_ctx(llc_ids[1]) : rand_llc_ctx();
  ...
  trace("PICK2 cpu[%d] first[%d] %llu second[%d] %llu", cpu, first->id,
        first->load, second->id, second->load);

  cur_load = cur_llcx->load + ((cur_llcx->load * lb_config.slack_factor) / 100);

  if (first->load >= cur_load && consume_llc(first))
    return 0;

  if (second->load >= cur_load && consume_llc(second))
    return 0;
...
static __always_inline void p2dq_dispatch_impl(s32 cpu,
  ...
  if (dsq_id != 0)
    trace("DISPATCH cpu[%d] min_vtime %llu dsq_id %llu atq %llu", cpu,
          min_vtime, dsq_id, min_atq);
  ...
  if (dsq_id != cpuc->affn_dsq && scx_bpf_dsq_move_to_local(cpuc->affn_dsq))
    return;

  if (dsq_id != cpuc->llc_dsq && scx_bpf_dsq_move_to_local(cpuc->llc_dsq))
    return;
  ...
  if (p2dq_config.atq_enabled) {
    ...
    if (dsq_id != cpuc->mig_dsq && scx_bpf_dsq_move_to_local(cpuc->mig_dsq))
      return;
  }
...
void BPF_STRUCT_OPS(p2dq_set_cpumask, struct task_struct *p,
  ...
  taskc->all_cpus =
      p->cpus_ptr == &p->cpus_mask && p->nr_cpus_allowed == topo_config.nr_cpus;
}
...
void BPF_STRUCT_OPS(p2dq_update_idle, s32 cpu, bool idle) {
  ...
  if (!idle || !p2dq_config.cpu_priority || !(llcx = lookup_cpu_llc_ctx(cpu)))
    return;
...
p2dq_init_task_impl(struct task_struct *p, struct scx_init_task_args *args) {
  struct mask_wrapper *wrapper;
  struct bpf_cpumask *cpumask;
  task_ctx *taskc;
  struct cpu_ctx *cpuc;
  ...
  wrapper =
      bpf_task_storage_get(&task_masks, p, 0, BPF_LOCAL_STORAGE_GET_F_CREATE);
  if (!wrapper) {
    bpf_cpumask_release(cpumask);
  ...
  slice_ns =
      scale_by_task_weight(p, dsq_time_slice(p2dq_config.init_dsq_index));
...
static s32 init_cpu(int cpu) {
  ...
  // Enhanced topology information is populated from Rust
  ...
  trace("CFG CPU[%d]NODE[%d]LLC[%d] initialized", cpu, cpuc->node_id,
        cpuc->llc_id);
...
static bool load_balance_timer(void) {
  ...
  bpf_for(llc_index, 0, topo_config.nr_llcs) {
    ...
    u32 lb_slack = (lb_config.slack_factor > 0 ? lb_config.slack_factor
                                               : LOAD_BALANCE_SLACK);
    ...
    dbg("LB llcx[%u] %llu lb_llcx[%u] %llu imbalance %lli", llc_id, llcx->load,
        lb_llc_id, lb_llcx->load, load_imbalance);
  }
...
static int timer_cb(void *map, int key, struct timer_wrapper *timerw) {
  ...
  bpf_timer_start(&timerw->timer, cb_timer->interval_ns, cb_timer->start_flags);
...
static __always_inline s32 p2dq_init_impl() {
  ...
  bpf_for(i, 0, topo_config.nr_cpus) {
    ...
    if (cpuc && llcx->node_cpumask && llcx->node_id == cpuc->node_id) {
      bpf_rcu_read_lock();
...
/**
 * BPF program to update the CPU context with enhanced topology information.
 * This function is called from Rust to directly update the cpu_ctx structure
 * without using global arrays.
 */
SEC("syscall")
s32 update_cpu_topology(struct update_cpu_topology_args *args) {
  struct cpu_ctx *cpuc;

  if (!args)
    return -EINVAL;

  if (!(cpuc = lookup_cpu_ctx(args->cpu_id)))
    return -ENOENT;

  // Enhanced topology information
  cpuc->core_id = args->core_id;
  cpuc->package_id = args->package_id;
  cpuc->cluster_id = args->cluster_id;
  cpuc->smt_level = args->smt_level;
  cpuc->cpu_capacity = args->cpu_capacity;

  // Cache information
  cpuc->l2_id = args->l2_id;
  cpuc->l3_id = args->l3_id;
  cpuc->cache_size = args->cache_size;

  // Frequency information
  cpuc->min_freq = args->min_freq;
  cpuc->max_freq = args->max_freq;
  cpuc->base_freq = args->base_freq;

  // Power management
  cpuc->pm_qos_resume_latency_us = args->pm_qos_resume_latency_us;
  cpuc->trans_lat_ns = args->trans_lat_ns;

  return 0;
}

/**
 * BPF program to update the LLC context with enhanced topology information.
 * This function is called from Rust to directly update the llc_ctx structure
 * without using global arrays.
 */
SEC("syscall")
s32 update_llc_topology(struct update_llc_topology_args *args) {
  struct llc_ctx *llcx;

  if (!args)
    return -EINVAL;

  if (!(llcx = lookup_llc_ctx(args->llc_id)))
    return -ENOENT;

  // Enhanced topology information
  llcx->kernel_id = args->kernel_id;
  llcx->cache_level = args->cache_level;
  llcx->cache_size = args->cache_size;
  llcx->cache_line_size = args->cache_line_size;
  llcx->ways_of_associativity = args->ways_of_associativity;
  llcx->physical_line_partition = args->physical_line_partition;
  llcx->coherency_line_size = args->coherency_line_size;
  llcx->nr_cores = args->nr_cores;
  llcx->nr_siblings = args->nr_siblings;

  return 0;
}

/**
 * BPF program to update the Node context with NUMA distance information.
 * This function is called from Rust to directly update the node_ctx structure
 * without using global arrays.
 */
SEC("syscall")
s32 update_node_topology(struct update_node_topology_args *args) {
  struct node_ctx *nodec;
  int i;
  ...
  if (!(nodec = lookup_node_ctx(args->node_id)))
    return -ENOENT;

  // NUMA distance information
  nodec->nr_nodes = args->nr_nodes;

  // Copy distance array
  for (i = 0; i < args->nr_nodes && i < MAX_NUMA_NODES; i++) {
    nodec->distance[i] = args->distance[i];
  }
...
s32 BPF_STRUCT_OPS_SLEEPABLE(p2dq_init) { return p2dq_init_impl(); }
...
SCX_OPS_DEFINE(p2dq, .select_cpu = (void *)p2dq_select_cpu,
               ...
               .exit = (void *)p2dq_exit, .timeout_ms = 20000, .name = "p2dq");
#endif
