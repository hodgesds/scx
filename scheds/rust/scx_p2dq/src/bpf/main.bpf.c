						  ...
						  struct llc_ctx *llcx)
{
	...
	if (llcx->target_load_sum > requested_sum) {
		return dsq_slice;
	}
	u64 slice_ns = (dsq_slice * ((100 * llcx->target_load_sum) / requested_sum)) / 100;
	if (dsq_index > 0)
		slice_ns >>= dsq_index;
	// dbg("deadline target_sum: %llu requested sum: %llu", llcx->target_load_sum, requested_sum);
	dbg("deadline ns[%d]: %llu %llu", dsq_index, slice_ns, dsq_slice);
...
static __always_inline s32 p2dq_init_impl()
{
	...
	bpf_for(llc_id, 0, nr_llcs) {
		...
		bpf_for(i, 0, nr_dsqs_per_llc) {
			...
			llcx->vtime = 0;
			llcx->target_load_sum = target_latency_ns * llcx->nr_cpus;
		}
	}
	struct cpu_ctx *cpuc;
	bpf_for(i, 0, nr_cpus) {
