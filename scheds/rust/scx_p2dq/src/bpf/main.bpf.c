			 ...
			 s32 prev_cpu, u64 wake_flags, bool *is_idle)
{
	 ...
	 * affinity mask and has no queued work. Fall back to normal path if not.
	 */
	if (!saturated && !overloaded && !(wake_flags & SCX_WAKE_SYNC)) {
		s32 waker_cpu = bpf_get_smp_processor_id();
	...
	/* Get SMT mask only if we need to search beyond prev_cpu */
	idle_smtmask = scx_bpf_get_idle_smtmask();
	if (!idle_smtmask)
		goto found_cpu;
