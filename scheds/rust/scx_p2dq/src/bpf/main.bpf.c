...
static __always_inline u64 shard_dsq_id(u32 llc_id, u32 shard_id) {
  return ((MAX_DSQS_PER_LLC * MAX_LLCS) << 3) + (llc_id * MAX_DSQS_PER_LLC) +
         shard_id;
}

static __always_inline u32 cpu_to_shard_id(struct llc_ctx *llcx, s32 cpu) {
  if (!llcx || p2dq_config.cpu_shards == 0 || llcx->nr_shard_dsqs == 0)
    return 0;

  return llcx->cpu_to_shard[cpu] % llcx->nr_shard_dsqs;
}

static __always_inline u64 get_llc_dsq(struct llc_ctx *llcx, s32 cpu) {
  if (!llcx)
    return 0;

  if (p2dq_config.cpu_shards == 0 || llcx->nr_shard_dsqs == 0)
    return llcx->dsq;

  u32 shard_id = cpu_to_shard_id(llcx, cpu);
  return llcx->shard_dsqs[shard_id];
}
...
static s32 pick_idle_cpu(struct task_struct *p, task_ctx *taskc, s32 prev_cpu,
  ...
  if (p2dq_config.sched_mode == MODE_PERF && topo_config.has_little_cores &&
      llcx->big_cpumask) {
...
static void async_p2dq_enqueue(struct enqueue_promise *ret,
  ...
  if (p2dq_config.select_idle_in_enqueue &&
    ...
    if (migrate) {
      ...
      taskc->dsq_id = get_llc_dsq(llcx, cpu);
      ret->kind = P2DQ_ENQUEUE_PROMISE_VTIME;
      ret->vtime.dsq_id = taskc->dsq_id;
      ret->vtime.slice_ns = taskc->slice_ns;
      ret->vtime.enq_flags = enq_flags;
...
static int dispatch_pick_two(s32 cpu, struct llc_ctx *cur_llcx,
                             struct cpu_ctx *cpuc) {
...
static void p2dq_dispatch_impl(s32 cpu, struct task_struct *prev) {
  ...
  // Handle sharded DSQs - try to dispatch from all shards if sharding is
  // enabled
  if (p2dq_config.cpu_shards > 0 && (llcx = lookup_llc_ctx(cpuc->llc_id)) &&
      llcx->nr_shard_dsqs > 0) {
    ...
    for (u32 shard_idx = 0; shard_idx < llcx->nr_shard_dsqs; shard_idx++) {
      u64 shard_dsq = llcx->shard_dsqs[shard_idx];
      if (shard_dsq != cpuc->llc_dsq && shard_dsq != dsq_id &&
          scx_bpf_dsq_move_to_local(shard_dsq))
        return;
    }
  } else {
    // Non-sharded mode - use the default LLC DSQ
    if (dsq_id != cpuc->llc_dsq && scx_bpf_dsq_move_to_local(cpuc->llc_dsq))
      return;
  }
...
void BPF_STRUCT_OPS(p2dq_update_idle, s32 cpu, bool idle) {
  ...
  if (saturated) {
    ...
    min_llc_runs_pick2 =
        min(log2_u32(percent_idle) + llc_scaler, lb_config.min_llc_runs_pick2);
  }
  ...
  if (idle)
    ...
    bpf_cpumask_and(llcx->tmp_cpumask, cast_mask(llcx->cpumask), idle_cpumask);
    if (llcx->tmp_cpumask &&
...
static int init_llc(u32 llc_index) {
  ...
  llcx->nr_shard_dsqs = 0;
  bpf_for(i, 0, MAX_DSQS_PER_LLC) { llcx->shard_dsqs[i] = 0; }
  bpf_for(i, 0, MAX_CPUS) { llcx->cpu_to_shard[i] = 0; }

  // If CPU sharding is enabled, initialize the sharded DSQs
  if (p2dq_config.cpu_shards > 0) {
    llcx->nr_shard_dsqs = min(p2dq_config.cpu_shards, MAX_DSQS_PER_LLC);

    for (u32 i = 0; i < llcx->nr_shard_dsqs; i++) {
...
static s32 p2dq_init_impl() {
  ...
  bpf_for(i, 0, topo_config.nr_cpus) {
    ...
    // If CPU sharding is enabled, set up the cpu to shard mapping
    if (p2dq_config.cpu_shards > 0 && llcx->nr_shard_dsqs > 0) {
      // Distribute CPUs evenly across shards within the LLC
      u32 shard_id = cpuc->id % llcx->nr_shard_dsqs;
      llcx->cpu_to_shard[cpuc->id] = shard_id;
      cpuc->llc_dsq = llcx->shard_dsqs[shard_id];
    } else {
