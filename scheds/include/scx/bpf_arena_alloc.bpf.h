...
#ifndef __SCX_BPF_ARENA_ALLOC_H
#define __SCX_BPF_ARENA_ALLOC_H
...
static inline void __arena* bpf_alloc(unsigned int size)
{
	...
	if (!page) {
refill:
		page = (void __arena *)bpf_arena_alloc_pages(&arena, NULL, 1, NUMA_NO_NODE, 0);
		if (!page)
			return NULL;
		cast_kern(page);
...
static inline void bpf_free(void __arena *addr)
{
	...
	if (--(*obj_cnt) == 0)
		bpf_arena_free_pages(&arena, (void *)addr, 1);
}
#else
static inline void __arena* bpf_alloc(unsigned int size) { return NULL; }
static inline void bpf_free(void __arena *addr) {}
#endif
#endif /* __SCX_BPF_ARENA_ALLOC_H */
